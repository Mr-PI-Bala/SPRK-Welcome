# SPRK-Welcome

Public doorway for SPRK students to learn how to join, request access, and start using SPRK repositories.

SPRK is the student-focused Coding Club in the AURAVYBE context. This repository is the public front door. It explains the student path before a student receives access to private working repositories such as `SPRK-Hello-Ursina`.

## Start Here

- Official doorway: `https://github.com/Mr-PI-Bala/SPRK-Welcome`
- First working project: `https://github.com/Mr-PI-Bala/SPRK-Hello-Ursina`
- [Create or convert a SPRK GitHub account](docs/SPRK_Git_Repository_UserGuide.md#sprk-github-account-pattern)
- [Request access to a working repository](docs/SPRK_Git_Repository_UserGuide.md#request-access-to-a-working-repository)
- [Use Codespaces](docs/SPRK_CodeSpaces_UserGuide.md)
- [Create your branch](docs/SPRK_Git_Repository_UserGuide.md#create-your-branch)
- [Test classroom devices and networks](docs/SPRK_Classroom_Network_Test_Guide.md)

If a working repository is private, request access through the process described in [docs/USER_GUIDE.md](docs/USER_GUIDE.md).

## Quick Start Guide

- Create or convert your SPRK GitHub account using the `FirstName-SPRK` username pattern.
- Open this welcome repository, then open the working project you want to use.
- Request access with a GitHub issue if the working repository is private or you cannot push your branch.
- Open Codespaces, read the working repository `README.md`, and run the safe project check.
- Create your own branch, commit your work, push it, and open a Pull Request for review.
- For group activities, use the classroom network test guide before students join from phones, Chromebooks, or tablets.

## SPRK Path

```text
Create or convert a GitHub account
  |
  v
Read SPRK-Welcome
  |
  v
Request access to a working repository
  |
  v
Open Codespaces
  |
  v
Run the safe project check
  |
  v
Create your branch
  |
  v
Make change, commit, push, and open a Pull Request
```

## Project Documents

- [docs/USER_GUIDE.md](docs/USER_GUIDE.md): public doorway guide for starting from SPRK-Welcome.
- [docs/SPRK_Git_Repository_UserGuide.md](docs/SPRK_Git_Repository_UserGuide.md): master GitHub access, branch, commit, push, and pull request guide.
- [docs/SPRK_CodeSpaces_UserGuide.md](docs/SPRK_CodeSpaces_UserGuide.md): master Codespaces runtime, safe check, and graphics limit guide.
- [docs/SPRK_Classroom_Network_Test_Guide.md](docs/SPRK_Classroom_Network_Test_Guide.md): classroom backend-host, device, and network validation guide.
- [docs/SPRK_VSCode_UserGuide.md](docs/SPRK_VSCode_UserGuide.md): master VS Code, Markdown, Mermaid, and extension guide.
- [docs/PROJECT_GUIDE.md](docs/PROJECT_GUIDE.md): maintainer guide for roles, governance, baseline repository structure, and approval flow.
- [docs/CHANGELOG.md](docs/CHANGELOG.md): release history.
- [LICENSE](LICENSE): current repository license notice.
- [VERSION](VERSION): current version.

## Current Version

`0.0.7`
